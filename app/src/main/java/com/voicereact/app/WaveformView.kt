...
class VrmTestActivity : Activity() {
    ...
    // Leg bone entities found by name, used to break the flat T-pose stance
    // with a static forward-step offset applied once after load.
    private var rightUpperLegEntity: Int = 0
    private var hasRightUpperLeg = false
    private var leftLowerLegEntity: Int = 0
    private var hasLeftLowerLeg = false
    ...
    // Medium-long 3/4 framing: knee/mid-calf up to just above the head,
    // character filling roughly 75-80% of the vertical frame, with a
    // ~10% safety margin above the head/arms for blend-shape and bone
    // animation clipping.
    private val cameraDistance = 0.60
    private val cameraLookHeight = 0.12 // shifts the focal center up toward chest/upper-body
    ...
    // Medium-long 3/4 framing: distance and vertical look height tuned so
    // the character spans ~75-80% of the vertical viewport, cropped from
    // roughly mid-calf/knee up to just above the head, with a ~10%
    // headroom margin so animated bones/blend shapes never clip the frame.
    private fun applyFullScreenFraming() {
        val viewer = modelViewer ?: return
        try {
            val camera: Camera = viewer.camera
            camera.lookAt(
                0.0, cameraLookHeight, cameraDistance,
                0.0, cameraLookHeight, 0.0,
                0.0, 1.0, 0.0
            )
        } catch (e: Throwable) {
        }
    ...
    private fun updateCameraDrift(elapsedSeconds: Double) {
        ...
        try {
            ...
            val eyeZ = cameraDistance + zoomDrift
            val eyeX = panDrift
            camera.lookAt(eyeX, cameraLookHeight, eyeZ, 0.0, cameraLookHeight, 0.0, 0.0, 1.0, 0.0)
        } catch (e: Throwable) {
        }
    }

    // Search the model's node hierarchy by name for standard VRM/humanoid
    // leg bone names (case-insensitive, works across naming conventions
    // like "RightUpperLeg", "J_Bip_R_UpperLeg", "rightupperleg", etc.)
    private fun findLegBones() {
        val viewer = modelViewer ?: return
        ...
        try {
            ...
            for (entity in asset.entities) {
                ...
                val name = try { asset.getName(entity) } catch (e: Exception) { null } ?: continue
                val lower = name.lowercase()
                if (!hasRightUpperLeg && (lower.contains("rightupperleg") || lower.contains("r_upperleg") || (lower.contains("right") && lower.contains("upperleg")))) {
                    rightUpperLegEntity = entity
                    hasRightUpperLeg = true
                }
                if (!hasLeftLowerLeg && (lower.contains("leftlowerleg") || lower.contains("l_lowerleg") || (lower.contains("left") && lower.contains("lowerleg")))) {
                    leftLowerLegEntity = entity
    ...
    // Injects a one-time static idle pose: right thigh rotated slightly
    // forward and left shin bent slightly back, so the stance reads as a
    // natural weight-shifted step instead of a flat, frozen T-pose.
    private fun applyIdleStepPose() {
        val viewer = modelViewer ?: return
        try {
            val tm = viewer.engine.transformManager

            if (hasRightUpperLeg) {
                val instance = tm.getInstance(rightUpperLegEntity)
                if (instance != 0) {
                    val current = FloatArray(16)
                    tm.getTransform(instance, current)
                    val rot = FloatArray(16)
                    Matrix.setIdentityM(rot, 0)
                    Matrix.rotateM(rot, 0, -14f, 1f, 0f, 0f) // step forward
                    val result = FloatArray(16)
            ...
            if (hasLeftLowerLeg) {
                val instance = tm.getInstance(leftLowerLegEntity)
                if (instance != 0) {
                    val current = FloatArray(16)
                    tm.getTransform(instance, current)
                    val rot = FloatArray(16)
                    Matrix.setIdentityM(rot, 0)
                    Matrix.rotateM(rot, 0, 10f, 1f, 0f, 0f) // slight bend, weight shifted back
                    val result = FloatArray(16)
                    Matrix.multiplyMM(result, 0, current, 0, rot, 0)
                    tm.setTransform(instance, result)
                }
