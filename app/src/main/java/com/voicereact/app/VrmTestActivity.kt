...
class VrmTestActivity : Activity() {
    ...
    // FIX: camera distance is now the single source of truth for framing —
    // one constant controls how large the character reads on screen.
    private val baseCameraDistance = 0.62
    ...
    // FIX: much closer camera distance (was 1.35, now 0.62) so the model
    // fills the full vertical screen edge-to-edge instead of three-quarters.
    ...
    private fun updateCameraDrift(elapsedSeconds: Double) {
        ...
        try {
            val camera: Camera = viewer.camera
            val zoomDrift = sin(elapsedSeconds * 0.18) * 0.04
            val panDrift = sin(elapsedSeconds * 0.12) * 0.02
