...
class VrmTestActivity : Activity() {
    ...
    // Every found mouth-related morph target gets driven together, so we
    // don't depend on guessing one exact name across all 15 models.
    private var mouthEntities: MutableList<Pair<Int, Int>> = mutableListOf() // entity to morphIndex
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        try {
            ...
            // Real random pick, 1 of 15 — matches Page 2's spec exactly
    ...
    // FIX: VRoid-exported VRM models commonly name mouth blend shapes like
    // "Fcl_MTH_A", "vrc.v_a", "MTH_A", etc. — not a plain "A" or "mouth".
    // We now scan every entity's morph names and collect every plausible
    // mouth-open shape instead of requiring one exact name.
