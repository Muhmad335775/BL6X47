...
class VrmTestActivity : Activity() {
    ...
    @Volatile private var livePitchHz: Float = 150f // neutral default
    ...
    // Reaction state: NONE, SHOCK ("wow"/"really"), LAUGH ("haha"/"funny")
    ...
    // Mood lighting: high/fast pitch -> warm red/orange; low/calm pitch -> cool blue/purple.
    // Applied as a live light color change on the scene, not a post-process filter.
    private fun updateMoodLighting() {
        val viewer = modelViewer ?: return
        try {
            val lm = viewer.engine.lightManager
            val pitch = livePitchHz.coerceIn(80f, 350f)
            val warmth = ((pitch - 80f) / (350f - 80f)).coerceIn(0f, 1f) // 0 = cool, 1 = warm

            // Cool: blue/purple. Warm: red/orange.
    ...
    private fun updateIdleMotion(elapsedSeconds: Double) {
        ...
        try {
            ...
            if (currentReaction == Reaction.LAUGH && reactionStartSeconds >= 0) {
                ...
                extraTilt = (sin(t * 18.0) * 6.0).toFloat() // fast shake
            } else if (currentReaction == Reaction.SHOCK && reactionStartSeconds >= 0) {
                extraTilt = -4f // small head pull-back tilt
            }
    ...
    // Drives the reaction blend shape (if the model has one) while a
    // reaction is active, then clears it when the reaction window ends.
    ...
    // Best-effort on-device speech recognition. Uses Android's built-in
    // SpeechRecognizer; true guaranteed-offline behavior depends on what
    // recognition service and language packs are installed on the device.
    ...
    private fun restartSpeechListening() {
        if (!speechListeningActive) return
        try {
    ...
    // Simple autocorrelation-based pitch estimate for the mood-lighting color.
    private fun estimatePitchHz(buffer: ShortArray, length: Int, sampleRate: Int): Float {
        val minLag = sampleRate / 500 // 500 Hz upper bound
        val maxLag = sampleRate / 60  // 60 Hz lower bound
