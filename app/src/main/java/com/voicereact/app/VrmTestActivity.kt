...
class VrmTestActivity : Activity() {
    ...
    // Full-screen edge-to-edge framing: camera pulled in close and centered
    // on the upper body so the character fills the entire vertical viewport.
    private fun fitFullScreenFraming() {
        val viewer = modelViewer ?: return
        try {
            val camera: Camera = viewer.camera
            camera.lookAt(0.0, 0.05, 1.35, 0.0, 0.05, 0.0, 0.0, 1.0, 0.0)
        } catch (e: Throwable) {
        }
    }

    // Slow, continuous camera drift (soft zoom/pan) so the shot never
    // looks like a static screenshot.
    private fun updateCameraDrift(elapsedSeconds: Double) {
        val viewer = modelViewer ?: return
        try {
            val camera: Camera = viewer.camera
            val zoomDrift = sin(elapsedSeconds * 0.18) * 0.08
            val panDrift = sin(elapsedSeconds * 0.12) * 0.04
            val eyeZ = 1.35 + zoomDrift
            val eyeX = panDrift
            camera.lookAt(eyeX, 0.05, eyeZ, 0.0, 0.05, 0.0, 0.0, 1.0, 0.0)
        } catch (e: Throwable) {
        }
    }

    // Idle body motion: subtle breathing (vertical scale pulse) and a
    // gentle side-to-side sway applied to the whole root transform, so the
    // character never freezes like a statue even without bone rigging data.
    ...
    // Periodic natural blink cycle: eyes mostly open, quick close every
    // few seconds, so the face reads as alive.
