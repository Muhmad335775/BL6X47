...
class VrmTestActivity : Activity() {
    ...
    // REMOVED: Android's SpeechRecognizer entirely. It plays two audible
    // system tones (start/end) on every use that cannot be silenced from
    // app code — that was the exact "two beeps on release" you heard.
    // The "wow"/"haha" trigger-word reaction will come back later using
    // pure audio analysis instead, which makes no system sound at all.
