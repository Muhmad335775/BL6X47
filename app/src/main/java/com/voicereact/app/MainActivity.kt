...
class MainActivity : AppCompatActivity(), RecorderButtonView.Listener {
    ...
    // FIX: only start recording automatically after permission is granted
    // if the request was caused by an actual button press — never on the
    // silent first-launch permission request.
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (!hasMicPermission() && Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            // Silent first-launch request — recording must NOT auto-start from this.
