package com.voicereact.app

import android.Manifest
import android.content.Intent
import android.content.pm.PackageManager
import android.media.MediaRecorder
import android.net.Uri
import android.os.Build
import android.os.Bundle
import android.os.Handler
import android.os.Looper
import android.provider.Settings
import android.widget.TextView
import android.widget.Toast
import androidx.appcompat.app.AppCompatActivity
import androidx.core.app.ActivityCompat
import androidx.core.content.ContextCompat
import java.io.File
import java.util.Locale

class MainActivity : AppCompatActivity(), RecorderButtonView.Listener {

    private lateinit var micButton: RecorderButtonView
    private lateinit var waveform: WaveformView
    private lateinit var timerText: TextView
    private lateinit var cancelHint: TextView
    private lateinit var trashView: TrashView

    private var recorder: MediaRecorder? = null
    private var outputFile: File? = null
    private var elapsedSeconds = 0
    private val maxSeconds = 20
    // FIX: below this duration, MediaCodec/MediaExtractor frequently fail
    // to decode the clip at all on Page 3, which looked like the whole
    // pipeline "breaking". Reject too-short recordings right here instead.
    private val minRecordingMs = 700L
    private var recordingStartMs = 0L
    private val tickHandler = Handler(Looper.getMainLooper())
    private var running = false
    private var permissionRequestInFlight = false
    private var permissionRequestCameFromPress = false

    private val requestMicPermission =
        registerForActivityResult(androidx.activity.result.contract.ActivityResultContracts.RequestPermission()) { granted ->
            permissionRequestInFlight = false
            if (granted) {
                if (permissionRequestCameFromPress) {
                    startRecording()
                }
            } else {
                showPermissionDenied()
            }
            permissionRequestCameFromPress = false
        }

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_main)

        micButton = findViewById(R.id.micButton)
        waveform = findViewById(R.id.waveform)
        timerText = findViewById(R.id.timerText)
        cancelHint = findViewById(R.id.cancelHint)
        trashView = findViewById(R.id.trashView)

        micButton.listener = this

        if (!hasMicPermission() && Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            requestPermissionSafely(fromPress = false)
        }
    }

    private fun hasMicPermission(): Boolean {
        return ContextCompat.checkSelfPermission(this, Manifest.permission.RECORD_AUDIO) ==
            PackageManager.PERMISSION_GRANTED
    }

    private fun requestPermissionSafely(fromPress: Boolean) {
        if (permissionRequestInFlight) return
        permissionRequestInFlight = true
        permissionRequestCameFromPress = fromPress
        try {
            requestMicPermission.launch(Manifest.permission.RECORD_AUDIO)
        } catch (e: Exception) {
            permissionRequestInFlight = false
            permissionRequestCameFromPress = false
        }
    }

    override fun onPressStart() {
        if (!hasMicPermission()) {
            requestPermissionSafely(fromPress = true)
            return
        }
        startRecording()
    }

    override fun onDragProgress(progress: Float) {
        cancelHint.alpha = 1f - progress
        cancelHint.translationX = -(progress * dpToPx(30f))
        if (progress > 0f) {
            trashView.visibility = TextView.VISIBLE
            trashView.progress = progress
        } else {
            trashView.visibility = TextView.INVISIBLE
            trashView.progress = 0f
        }
    }

    override fun onSwipeCancel() {
        stopRecordingInternal(delete = true)
    }

    override fun onRelease(committed: Boolean) {
        stopRecordingInternal(delete = !committed)
    }

    private fun startRecording() {
        if (running) return
        if (!hasMicPermission()) return

        val clipsDir = File(filesDir, "clips").apply { mkdirs() }
        val file = File(clipsDir, "clip_${System.currentTimeMillis()}.m4a")
        outputFile = file

        val newRecorder = MediaRecorder()
        try {
            newRecorder.setAudioSource(MediaRecorder.AudioSource.MIC)
            newRecorder.setOutputFormat(MediaRecorder.OutputFormat.MPEG_4)
            newRecorder.setAudioEncoder(MediaRecorder.AudioEncoder.AAC)
            newRecorder.setAudioSamplingRate(44100)
            newRecorder.setAudioEncodingBitRate(128000)
            newRecorder.setOutputFile(file.absolutePath)
            newRecorder.prepare()
            newRecorder.start()
        } catch (e: Exception) {
            Toast.makeText(this, "Could not start recording", Toast.LENGTH_SHORT).show()
            try { newRecorder.release() } catch (ignored: Exception) {}
            return
        }
        recorder = newRecorder

        elapsedSeconds = 0
        recordingStartMs = System.currentTimeMillis()
        running = true
        micButton.setRecordingState(true)
        waveform.reset()
        timerText.text = formatTime(0)
        cancelHint.alpha = 1f
        cancelHint.translationX = 0f
        trashView.progress = 0f
        trashView.visibility = TextView.INVISIBLE
        timerText.visibility = TextView.VISIBLE
        waveform.visibility = TextView.VISIBLE
        cancelHint.visibility = TextView.VISIBLE

        tickHandler.post(tickRunnable)
        tickHandler.postDelayed(secondRunnable, 1000)
    }

    private val tickRunnable = object : Runnable {
        override fun run() {
            if (!running) return
            val amp = try {
                recorder?.maxAmplitude ?: 0
            } catch (e: Exception) {
                0
            }
            waveform.pushAmplitude(if (amp > 0) amp else WaveformView.simulatedTick())

            if (elapsedSeconds >= maxSeconds) {
                stopRecordingInternal(delete = false)
                return
            }
            tickHandler.postDelayed(this, 200)
        }
    }

    private val secondRunnable = object : Runnable {
        override fun run() {
            if (!running) return
            elapsedSeconds += 1
            timerText.text = formatTime(elapsedSeconds)
            if (elapsedSeconds < maxSeconds) {
                tickHandler.postDelayed(this, 1000)
            }
        }
    }

    private fun stopRecordingInternal(delete: Boolean) {
        if (!running) return
        running = false
        micButton.setRecordingState(false)
        tickHandler.removeCallbacksAndMessages(null)

        val actualDurationMs = System.currentTimeMillis() - recordingStartMs

        try {
            recorder?.stop()
        } catch (e: Exception) {
        }
        try {
            recorder?.release()
        } catch (e: Exception) {
        }
        recorder = null

        timerText.visibility = TextView.INVISIBLE
        waveform.visibility = TextView.INVISIBLE
        cancelHint.visibility = TextView.INVISIBLE
        cancelHint.alpha = 1f
        cancelHint.translationX = 0f
        trashView.visibility = TextView.INVISIBLE
        trashView.progress = 0f

        // FIX: too-short clips reliably fail to decode later — discard
        // them here with a clear message instead of letting Page 3 crash.
        if (!delete && actualDurationMs < minRecordingMs) {
            outputFile?.delete()
            outputFile = null
            Toast.makeText(this, "Hold the button a bit longer to record", Toast.LENGTH_SHORT).show()
            return
        }

        if (delete) {
            outputFile?.delete()
            outputFile = null
        } else {
            outputFile?.let { onClipReady(it) }
        }
    }

    private fun onClipReady(file: File) {
        val intent = Intent(this, CinematicRecordingActivity::class.java)
        intent.putExtra(CinematicRecordingActivity.EXTRA_AUDIO_PATH, file.absolutePath)
        startActivity(intent)
    }

    private fun formatTime(seconds: Int): String {
        val m = seconds / 60
        val s = seconds % 60
        return String.format(Locale.US, "%d:%02d", m, s)
    }

    private fun dpToPx(value: Float): Float = value * resources.displayMetrics.density

    private fun showPermissionDenied() {
        val shouldShowRationale = ActivityCompat.shouldShowRequestPermissionRationale(
            this, Manifest.permission.RECORD_AUDIO
        )
        if (!shouldShowRationale) {
            Toast.makeText(this, R.string.mic_permission_denied, Toast.LENGTH_LONG).show()
            try {
                val intent = Intent(Settings.ACTION_APPLICATION_DETAILS_SETTINGS).apply {
                    data = Uri.fromParts("package", packageName, null)
                }
                startActivity(intent)
            } catch (e: Exception) {
            }
        } else {
            Toast.makeText(this, R.string.mic_permission_rationale, Toast.LENGTH_LONG).show()
        }
    }

    override fun onDestroy() {
        super.onDestroy()
        tickHandler.removeCallbacksAndMessages(null)
        try { recorder?.release() } catch (e: Exception) {}
        recorder = null
    }
}
