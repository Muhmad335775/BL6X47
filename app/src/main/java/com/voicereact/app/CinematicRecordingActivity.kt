...
// Page 3 (Full-Screen Cinematic Video): loads a live VRM model, drives it
// from the previously recorded audio clip, renders every frame through
// Filament into a MediaCodec encoder surface, encodes an AAC audio track
// (original speech + a short generated "echo signature" tone), muxes both
// into a real .mp4 file with MediaMuxer, then plays it back full-screen.
class CinematicRecordingActivity : Activity() {
    ...
    private lateinit rootLayout: FrameLayout
    ...
    private fun renderCinematicVideo(audioPath: String) {
        ...
        if (decodedAudio == null) {
            uiHandler.post { Toast.makeText(this, "Could not decode audio", Toast.LENGTH_LONG).show() }
            return
        }
        ...
        try {
            ...
            // Feed audio PCM into the audio encoder up front (small clips, fits in memory).
            thread(start = true) {
                try {
            ...
            for (frameIndex in 0 until totalFrames) {
                ...
                // Mouth amplitude driven by the actual recorded speech window
            ...
            Thread.sleep(300) // give the audio feeder thread a moment to finish queuing
    ...
    // Cinematic color grading: gentle contrast/saturation lift via
    // Filament's tone mapping pipeline.
    ...
    // ---------------------------------------------------------------
    // Audio decode / analysis / echo tone / encode
    // ---------------------------------------------------------------
    ...
    // Echo Signature: takes the last ~1-2s of the recorded voice, finds its
    // dominant pitch, and appends a short fading sine tone at that pitch —
    // a small personal audio "signature" at the end of the clip.
    private fun buildFullAudioWithEcho(audio: DecodedAudio, tailSeconds: Double): ShortArray {
        ...
        for (frame in 0 until framesTotal) {
            ...
            val envelope = (1.0 - (frame.toDouble() / framesTotal)).coerceIn(0.0, 1.0) // fade out
